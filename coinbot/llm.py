...
Write `Staged` to see all coins that are soon-to-be-added in the collection.
Write `Hoarders` to see a list of the most successful collectors.
...
class LLM:
    def __init__(
        ...
        self.model = model
        self.client = OpenAI(
            api_key=token, base_url="https://api.fireworks.ai/inference/v1"
        )
...
class Embedding:
    def __init__(self, token: str, model: str = "fireworks/qwen3-embedding-8b"):
        self.token = token
        self.model = model
        self.client = OpenAI(
            api_key=self.token, base_url="https://api.fireworks.ai/inference/v1"
        )

    def embed(self, message: str):
        response = self.client.embeddings.create(model=self.model, input=message)
